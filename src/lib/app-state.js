...
export function getGatewayService() {
...
export function syncGatewayStatus(services) {
  const nextGateway = getGatewayServiceSnapshot(services)
  _gatewayService = nextGateway
  if (nextGateway.running) _gwStopCount = 0
  _setGatewayRunning(nextGateway.running)
  return nextGateway
}
...
export async function detectOpenclawStatus() {
  try {
    ...
    const gateway = services.status === 'fulfilled'
      ? syncGatewayStatus(services.value)
...
export async function refreshGatewayStatus(options = {}) {
  try {
    const services = await api.getServicesStatus(options.force === true)
    if (services?.length > 0) {
      const gateway = getGatewayServiceSnapshot(services)
      _gatewayService = gateway
      const nowRunning = gateway.running
      if (nowRunning) {
        _gwStopCount = 0
        if (!_gatewayRunning) {
